...
class FileWritter:
    ...
    @property
    def file_path(self):
        return Path(self.log_path) / Path(DatetimeConverter().date_str + '.' + self._file_name)
...
class BulkFileWrite(FileWritter):
    ...
    def queue(self):
        with self._get_queue_lock:
            ...
            if (pid,self._file_name) not in self.__class__.pid_filename__queue_map:
                self.__class__.pid_filename__queue_map[(pid,self._file_name)] = queue.SimpleQueue()
            print(self.__class__.pid_filename__queue_map)
            return self.__class__.pid_filename__queue_map[(pid,self._file_name)]
        ...
        with self._lock:
            ...
            while 1:
                if not self.queue.empty():
                    msg_list.append(self.queue.get())
                else:
                    break
            if msg_list:
                self._close_file()
    ...
    def _cleanup_factory(self):
        ...
        def wrapper():
            try:
                ...
                print(f'要退出。。  {multiprocessing.current_process().name}')
                _self.close()
    ...
    def _unregister_cleanup(self):
        ...
        if getattr(self, '_cleanup', None):
            ...
            # py2 requires removing from private attribute...
                ...
                try:
                    print(666,atexit._exithandlers)
...
# print_file_writter = BulkFileWrite('xx7.test')
...
def tt1():
    print_file_writter = BulkFileWrite('xx8.test')
    print_file_writter.start_bulk_write()
    print_file_writter.need_write_2_file = True
    print(print_file_writter.need_write_2_file)
...
if __name__ == '__main__':
    ...
    multiprocessing.Process(target=tt1).start()
    time.sleep(0.3)
    print(BulkFileWrite.pid_filename__queue_map)
